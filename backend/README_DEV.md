...
- Локальная SQLite-БД: `data/ssau_schedule_bot.local.db`.
- Production SQLite-БД в Docker: `data/ssau_schedule_bot.prod.db`.
- `envs/sensitive.env` не коммитится. В нём должны лежать секреты:
  `TELEGRAM__BOT_TOKEN`, `SECURITY__FERNET_KEY`, `SECURITY__STATE_TOKEN_SECRET`,
  `API__ADMIN_API_TOKEN`.
- `local.env` и `dev.env` разрешают `SECURITY__ALLOW_PLAINTEXT=true`, но в `prod` нужен
  `SECURITY__FERNET_KEY`.
- `common.env` содержит только общие не-секретные дефолты.
- В Docker `VALKEY__HOST=valkey` задаётся в `docker-compose.yaml`, потому что из контейнера
  `127.0.0.1` указывает на сам контейнер приложения.
...
Если нужно запустить приложение полностью в Docker как production, явно передайте `ENV=prod`:
...
`VALKEY__HOST=valkey` из `docker-compose.yaml`. Проверьте, что сервис запущен через compose,
и пересоздайте `bot`, `worker`, `api`.

Swagger через nginx доступен на `https://auth.sasau.ru:8444/api/docs`.
Прямой адрес API: `http://<server-ip>:3100/docs`, если порт открыт:
