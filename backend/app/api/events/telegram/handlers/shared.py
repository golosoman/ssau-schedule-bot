...
async def safe_delete_message(message: Message) -> None:
    """Удаляет сообщение пользователя (напр. с кредами), молча игнорируя ошибки.

    Telegram разрешает боту удалять входящие сообщения в личке только если они
    отправлены менее 48 часов назад — старые/уже удалённые молча пропускаем.
    """
    try:
        await message.delete()
    except Exception:
        logger.debug("Failed to delete message %s in chat %s.", message.message_id, message.chat.id)
