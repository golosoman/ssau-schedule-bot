...
class NextJsLoginScraper:
    ...
    def extract_next_action(cls, js: str) -> str:
        ...
        assign_ids: list[str] = cls._ACTION_ID_ASSIGN_RE.findall(js)
        if len(assign_ids) == 1:
            return assign_ids[0]

        prop_ids: list[str] = cls._ACTION_ID_PROP_RE.findall(js)
        if len(prop_ids) == 1:
            return prop_ids[0]
        ...
        hashes: list[str] = cls._ACTION_HASH_RE.findall(js)
        if not hashes:
    ...
    def extract_next_action_from_html(cls, html_text: str) -> str | None:
        ...
        for pattern in (
            ...
            matches: list[str] = pattern.findall(html_text)
            if len(matches) == 1:
                return matches[0]

        hashes: list[str] = cls._ACTION_HASH_RE.findall(html_text)
        if len(hashes) == 1:
            return hashes[0]
        return None
    ...
    def extract_form_value(cls, html_text: str, field_name: str) -> str | None:
        ...
        textareas: list[tuple[str, str]] = cls._TEXTAREA_RE.findall(html_text)
        for name, value in textareas:
            if name == field_name:
                return html.unescape(value).strip()
        return None

    @classmethod
    def _iter_search_texts(cls, html_text: str) -> Iterator[Any]:
        yield html_text
    ...
    @classmethod
    def _extract_tree_from_text(cls, text: str) -> list[Any] | None:
        trees: list[list[Any]] = []
    ...
    @classmethod
    def _extract_all_trees(cls, text: str) -> list[list[Any]]:
        trees: list[list[Any]] = []
    ...
    @classmethod
    def _extract_flight_root_payload(cls, text: str) -> dict[str, Any] | None:
        for candidate in cls._iter_search_texts(text):
            if not isinstance(candidate, str):
                continue
    ...
    @classmethod
    def _extract_tree_from_flight(cls, text: str) -> list[Any] | None:
    ...
    @classmethod
    def _build_path_from_flight_segments(cls, segments: object) -> str | None:
    ...
    @classmethod
    def _normalize_router_state(cls, tree: list[Any], route_path: str | None) -> list[Any]:
        cleaned: list[Any] = cls._replace_undefined(tree)
    ...
    @classmethod
    def _replace_undefined(cls, value: Any) -> Any:
        if value == "$undefined":
            return None
    ...
    @classmethod
    def _ensure_layout_flags(cls, node: list[Any]) -> None:
        if not isinstance(node, list) or len(node) < 2:
            return
    ...
    @classmethod
    def _select_tree(cls, trees: list[list[Any]]) -> list[Any] | None:
        if not trees:
            return None
        for tree in trees:
    ...
    @classmethod
    def _get_page_path(cls, node: list[Any]) -> str | None:
        if not isinstance(node, list) or len(node) < 2:
            return None
        segment = node[0]
        if segment == "__PAGE__":
